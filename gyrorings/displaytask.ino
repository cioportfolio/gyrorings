...
int nextDisplaySensitivity = 1<<14;
int turnSensitivity = 1<<8;
...
/*    case newStart:
    ...
    case gyroSensitivity:
      turnSensitivity = 1<<command.b;
      break;
    case nextSensitivity:
      nextDisplaySensitivity = 1<<command.b;
    default:
      break;
    }
  }
...
void genDisplay()
{
  int16_t beat = calcEMA();
  //Serial.println(beat);
  if ((GyX/turnSensitivity > 0 && colStep < 0) || (GyX/turnSensitivity < 0 && colStep > 0))
  {
    colStep = -colStep;
  }
  ...
  ChangePalettePeriodically(beat);
  if (dispMode == 0)
  {
    FillLEDsFromPaletteColors(beat);
  }
  else
  {
...
void FillLEDsFromPaletteColors(int16_t beat)
{
  ...
  for (int i = 0; i < NUM_LEDS; i++)
  {
    ...
    leds[i] = ColorFromPalette(currentPalette, colOffset, glow, currentBlending);
  }
}
...
void ChangePalettePeriodically(int16_t beat)
{
  ...
  if (abs(GyY) > nextDisplaySensitivity)
  {
    beatCount=0;
    layout = random8() % 7;
    dir = random8() % 2 * 2 - 1;
